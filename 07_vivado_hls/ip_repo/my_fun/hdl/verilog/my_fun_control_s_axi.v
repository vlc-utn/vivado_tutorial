...
#(parameter
    ...
    output wire [31:0]                   x2,
    output wire [31:0]                   x3,
    input  wire [31:0]                   b_y1,
    input  wire                          b_y1_ap_vld,
    input  wire [31:0]                   y2,
    input  wire                          y2_ap_vld,
    input  wire [31:0]                   y3,
    input  wire                          y3_ap_vld,
...
localparam
    ...
    ADDR_IER         = 7'h08,
    ADDR_ISR         = 7'h0c,
    ADDR_X1_DATA_0   = 7'h10,
    ADDR_X1_CTRL     = 7'h14,
    ADDR_X2_DATA_0   = 7'h18,
    ADDR_X2_CTRL     = 7'h1c,
    ADDR_X3_DATA_0   = 7'h20,
    ADDR_X3_CTRL     = 7'h24,
    ADDR_B_Y1_DATA_0 = 7'h28,
    ADDR_B_Y1_CTRL   = 7'h2c,
    ADDR_Y2_DATA_0   = 7'h38,
    ADDR_Y2_CTRL     = 7'h3c,
    ADDR_Y3_DATA_0   = 7'h48,
    ADDR_Y3_CTRL     = 7'h4c,
    WRIDLE           = 2'd0,
    WRDATA           = 2'd1,
    WRRESP           = 2'd2,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_B_Y1_DATA_0: begin
                    rdata <= int_b_y1[31:0];
                end
                ADDR_B_Y1_CTRL: begin
                    rdata[0] <= int_b_y1_ap_vld;
                end
                ADDR_Y2_DATA_0: begin
                    rdata <= int_y2[31:0];
                end
                ADDR_Y2_CTRL: begin
                    rdata[0] <= int_y2_ap_vld;
                end
                ADDR_Y3_DATA_0: begin
                    rdata <= int_y3[31:0];
                end
                ADDR_Y3_CTRL: begin
                    rdata[0] <= int_y3_ap_vld;
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_b_y1_ap_vld <= 1'b0;
    else if (ACLK_EN) begin
        if (b_y1_ap_vld)
            int_b_y1_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_B_Y1_CTRL)
            int_b_y1_ap_vld <= 1'b0; // clear on read
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (y2_ap_vld)
            int_y2_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_Y2_CTRL)
            int_y2_ap_vld <= 1'b0; // clear on read
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (y3_ap_vld)
            int_y3_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_Y3_CTRL)
            int_y3_ap_vld <= 1'b0; // clear on read
    end
end
